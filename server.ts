...
app.post("/api/bot-chat", async (req, res) => {
  ...
  if (normalizedMsg === "/legal" || normalizedMsg.includes("документ") || normalizedMsg.includes("оферт") || normalizedMsg.includes("ооо") || normalizedMsg.includes("ип") || normalizedMsg.includes("реквизит")) {
    return res.json({
      reply: `📜 **Юридическая прозрачность и реквизиты:**\n\nВсе платежи проходят официально под контролем ЮKassa (ООО НКО "ЮМани") в соответствии с действующим законодательством РФ.\n\n📍 **Реквизиты продавца (Самозанятый):**\n• **Продавец:** Самозанятая Себиева Рояна\n• **ИНН:** 770980461804 (Налог на профессиональный доход - НПД)\n• **Банк:** ООО "ОЗОН БАНК"\n• **БИК банка:** 044525068\n• **Расчетный счет:** 40914810500038630929\n• **Электронная почта:** help@startappai.ru\n• **Сайт:** startappai.ru\n\n📁 **Важные документы:**\n1. **Публичная оферта** — Договор присоединения по ст. 437 ГК РФ. Содержит правила предоставления информационных услуг.\n2. **Условия возврата средств** — По Закону РФ «О защите прав потребителей» ст. 26.1 (дистанционный способ) возврат возможен до момента фактического открытия доступа к закрытому контенту.\n3. **Политика конфиденциальности** — Полное соответствие закону ФЗ №152.\n\nКаждая успешная транзакция автоматически формирует электронный кассовый чек по ФЗ-54 и отправляется покупателю.`,
      suggestedButtons: [
  ...
  // Dedicated YooKassa / YooMoney Moderation Guide Router
  if (normalizedMsg.includes("yoomoney") || normalizedMsg.includes("yookassa") || normalizedMsg.includes("юмани") || normalizedMsg.includes("юкасса") || normalizedMsg.includes("скриншот") || normalizedMsg.includes("модерац") || normalizedMsg.includes("регистрац")) {
    return res.json({
      reply: `💳 **Инструкция по успешному прохождению модерации в ЮKassa для домена startappai.ru:**

Для одобрения магазина ЮKassa запрашивает **скриншоты полного цикла оплаты и фискализации** клиентом. 

Мы полностью настроили и автоматизировали эту симуляцию под требования менеджеров ЮKassa:
1️⃣ **Экран выбора тарифа/подписки** (Вкладка «II. Симулятор», Шаг 3). Показывает тарифы и подписку.
2️⃣ **Форма оплаты ЮKassa с реквизитами Самозанятого** (Симулятор, Шаг 4). Заполненная форма карт РФ и Системы Быстрых Платежей (СБП) с реквизитами Самозанятая Себиева Рояна и привязкой к startappai.ru.
3️⃣ **Экран статуса успешного платежа** (Симулятор, Шаг 5). VIP-доступ открывается автоматически.
4️⃣ **Электронный Кассовый Чек (Закон ФЗ-54)** (Симулятор, Шаг 5 ➡️ вкладка «Чек ФЗ-54»). Это важнейший документ, содержащий ИНН 770980461804 продавца Себиевой Рояны, банк ООО "ОЗОН БАНК" и ФЗ-54 фискализацию через ЮMoney.

👉 **Инструкция как сделать скриншоты:**
1. Откройте вкладку **«II. Интерактивный Симулятор Воронки»**.
2. Перейдите на **Шаг 4 (Оплата)**, сделайте первый скриншот платежной формы со вводом карты.
3. Нажмите кнопку оплаты. На **Шаге 5 (Успешно)** нажмите на синюю вкладку **«📄 Чек ФЗ-54 (ЮKassa)»**.
4. Сделайте скриншот чека.
5. Отправьте эти два скриншота менеджеру в ЮKassa. Модерация у вас в кармане!`,
      suggestedButtons: [
        { text: "👉 Перейти к Симулятору", action: "miniapp" },
        { text: "📜 Посмотреть Реквизиты", action: "legal" }
      ]
    });
  }
...
Реквизиты оператора: Самозанятая Себиева Рояна, ИНН 770980461804, Банк ООО "ОЗОН БАНК", Расчетный счет 40914810500038630929, БИК 044525068.
...
3. Проконсультировать по налогам в России: самозанятость (НПД 4%), как платить, чтобы все было легально.
